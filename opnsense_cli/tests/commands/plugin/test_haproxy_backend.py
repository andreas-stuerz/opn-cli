...
class TestHaproxyBackendCommands(CommandTestCase):
    def setUp(self):
        ...
        self._api_data_fixtures_list_EMPTY = {"haproxy": {"backends": {"backend": []}}}
    ...
    def test_list(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            backend,
            ["list", "-o", "plain", "-c", "uuid,name,Servers,Resolver,Healthcheck,Mailer,Users,Groups,Actions,Errorfiles"],
        )
    ...
    def test_show(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            [
                "show",
                "2c57ff97-10df-41a1-8a02-ab2fd1a4a651",
                "-o",
                "plain",
                "-c",
        ...
        self.assertIn(("2c57ff97-10df-41a1-8a02-ab2fd1a4a651 pool2 server2,server4  http_head     \n"), result.output)
    ...
    def test_create_UNRESOLVED(self, api_response_mock: Mock):
        ...
        self.assertIn(("Error: Could not find uuid for $.haproxy.servers.server: ['non_existing_server']\n"), result.output)
        self.assertEqual(1, result.exit_code)

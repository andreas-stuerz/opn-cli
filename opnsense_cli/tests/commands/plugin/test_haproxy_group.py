...
from opnsense_cli.commands.plugin.haproxy.group import group
from opnsense_cli.tests.commands.base import CommandTestCase


class TestHaproxyGroupCommands(CommandTestCase):
    def setUp(self):
        ...
        self._api_data_fixtures_create_ERROR = {"result": "failed", "validations": {"group.members": "Related user not found"}}
        ...
        self._api_data_fixtures_list_EMPTY = {"haproxy": {"groups": {"group": []}}}
    ...
    @patch("opnsense_cli.commands.plugin.haproxy.group.ApiClient.execute")
    def test_list(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            group,
            ["list", "-o", "plain", "-c", "uuid,enabled,name,description,Users,add_userlist"],
        )

        self.assertIn(("741e1c18-ba34-4fab-823a-7017fc3af1ec 1 group1  user1,user2 1\n"), result.output)
    ...
    def test_show(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            [
                "show",
                "741e1c18-ba34-4fab-823a-7017fc3af1ec",
                "-o",
                "plain",
                "-c",
        ...
        self.assertIn(("1 group1  user1,user2 1\n"), result.output)
    ...
    def test_create_ERROR(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            [
                ...
                "my_test_group",
                "--members",
                "a99f0f32-7cab-49df-a2f8-a4fa6edab03f",
                "--description",
                "the gang",
            ],
        )
        ...
        self.assertIn(
            ("Error: {'result': 'failed', 'validations': " "{'group.members': 'Related user not found'}}\n"), result.output
        )
    ...
    def test_update_OK(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            group,
            ["update", "d99f0f32-7cab-49df-a2f8-a4fa6edab03f", "--description", "the cool gang"],
        )

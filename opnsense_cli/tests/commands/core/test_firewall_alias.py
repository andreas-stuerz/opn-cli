...
class TestFirewallAliasCommands(CommandTestCase):
    def setUp(self):
        self._api_data_fixtures_apply_OK = {"status": "ok"}
        self._api_data_fixtures_apply_FAILED = {"status": "failed"}
        self._api_data_fixtures_create_OK = {"result": "saved", "uuid": "5c2163a8-a429-4226-a3b8-dd2b1560b12b"}
        ...
        self._api_data_uuid_for_name = {"uuid": "23636461-dfd1-46cf-9d60-ee46f6aeb04a"}
    ...
    def test_show_(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            api_response_mock,
            [self._api_data_uuid_for_name, self._api_data_fixtures_list],
            alias,
    ...
    def test_table(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            api_response_mock, [self._api_data_fixtures_table], alias, ["table", "example_alias", "-o", "plain"]
        )
    ...
    def test_create_ERROR(self, api_response_mock: Mock):
        ...
        self.assertIn(
            ("Error: {'result': 'failed', 'validations': {'alias.content': " "'Entry \"test\" is not a valid alias.'}}\n"),
            result.output,
        )
    ...
    def test_create_EXISTS(self, api_response_mock: Mock):
        ...
        self.assertIn(
            ("Error: " "{'result': 'failed', 'validations': {'alias.name': 'An alias with this name already exists.'}}\n"),
            result.output,
        )
        self.assertEqual(1, result.exit_code)

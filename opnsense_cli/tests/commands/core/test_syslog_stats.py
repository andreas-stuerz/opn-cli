...
class TestSyslogStatsCommands(CommandTestCase):
    ...
    def test_list(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            stats,
            ["list", "-o", "plain", "-c", "#,SourceName,SourceInstance"],
        )
    ...
    def test_list_SEARCH(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            stats,
            ["list", "--search", "10.0.0.1", "-o", "plain", "-c", "#,SourceName,SourceInstance"],
        )

        self.assertIn(("e28eb856223592f3e11f8dcbb30f6dae dst.network tcp,10.0.0.1:514\n"), result.output)

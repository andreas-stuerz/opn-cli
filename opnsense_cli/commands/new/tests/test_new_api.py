...
class TestNewCommandApi(CommandTestCase):
    ...
    def test_plugin(self, mock_model_get: Mock):
        ...
        result = runner.invoke(
            api,
            ["plugin", "haproxy", "--api-reference-url", "https://fake.githubusercontent.com/plugin.html"],
            catch_exceptions=False,
        )
        self.assertIn(f"generate new code: {self._generated_plugin_api_path}\n", result.output)
        ...
        self.assertIn('self.method = "get"', api_plugin_file_content)

        self.assertIn('self.method = "post"', api_plugin_file_content)

        self.assertIn("@ApiBase._api_call", api_plugin_file_content)
        ...
        self.assertIn("def diff(self, *args):", api_plugin_file_content)

        self.assertIn('self.command = "diff"', api_plugin_file_content)
        ...
        self.assertIn("def certActions(self, *args):", api_plugin_file_content)

        self.assertIn('self.command = "certActions"', api_plugin_file_content)
        ...
        self.assertIn("def configtest(self, *args):", api_plugin_file_content)

        self.assertIn('self.command = "configtest"', api_plugin_file_content)
        ...
        self.assertIn("def addAcl(self, *args):", api_plugin_file_content)

        self.assertIn('self.command = "addAcl"', api_plugin_file_content)
        ...
        self.assertIn("def counters(self, *args):", api_plugin_file_content)
    ...
    def test_core(self, mock_model_get: Mock):
        ...
        result = runner.invoke(
            api,
            ["core", "cron", "--api-reference-url", "https://fake.githubusercontent.com/core.html"],
            catch_exceptions=False,
        )
        print(result.output)

        self.assertIn(f"generate new code: {self._generated_core_api_path}\n", result.output)
        ...
        self.assertIn('self.method = "get"', api_core_file_content)

        self.assertIn('self.method = "post"', api_core_file_content)

        self.assertIn("@ApiBase._api_call", api_core_file_content)
        ...
        self.assertIn("def reconfigure(self, *args):", api_core_file_content)

        self.assertIn('self.command = "reconfigure"', api_core_file_content)
        ...
        self.assertIn("def addJob(self, *args):", api_core_file_content)
    ...
    def test_list(self, mock_model_get: Mock):
        ...
        result_core = runner.invoke(
            api,
            ["list", "--module-type", "core", "--base-url", "https://fake.githubusercontent.com/module/core.html"],
            catch_exceptions=False,
        )
        self.assertIn("cron", result_core.output)

        self.assertIn("ipsec", result_core.output)

        self.assertIn("syslog", result_core.output)
        ...
        self.assertIn("postfix", result_plugin.output)

        self.assertIn("zabbixagent", result_plugin.output)

...
from opnsense_cli.commands.plugin.haproxy.healthcheck import healthcheck
from opnsense_cli.commands.test_base import CommandTestCase
import os

class TestHaproxyHealthcheckCommands(CommandTestCase):
    def setUp(self):
        ...
        self._api_data_fixtures_create_ERROR = {
            "result": "failed",
            "validations": {"healthcheck.checkport": "Please specify a value between 1 and 65535."},
        }
        ...
        self._api_data_fixtures_list_EMPTY = {"haproxy": {"healthchecks": {"healthcheck": []}}}
    ...
    @patch("opnsense_cli.commands.plugin.haproxy.healthcheck.ApiClient.execute")
    def test_list(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            healthcheck,
            ["list", "-o", "plain", "-c", "uuid,name,description,type,interval"],
        )

        self.assertIn(("5fc21dc7-5a0e-48ba-bc17-a7d41f5dadde http_head Monitor http http 2s\n"), result.output)
    ...
    def test_show(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            healthcheck,
            ["show", "5fc21dc7-5a0e-48ba-bc17-a7d41f5dadde", "-o", "plain", "-c", "uuid,name,description,type,interval"],
        )
    ...
    def test_create_OK(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            healthcheck,
            ["create", "my_test_healthcheck", "--type", "http"],
        )
    ...
    def test_create_ERROR(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            healthcheck,
            ["create", "my_test_healthcheck", "--checkport", 100000],
        )

        self.assertIn(
            (
                "Error: {'result': 'failed', 'validations': "
                "{'healthcheck.checkport': 'Please specify a value between 1 and 65535.'}}\n"
            ),
            result.output,
        )
    ...
    def test_update_OK(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            healthcheck,
            ["update", "5fc21dc7-5a0e-48ba-bc17-a7d41f5dadde", "--description", "updated by me"],
        )
    ...
    def test_update_NOT_EXISTS(self, api_response_mock: Mock):
        result = self._opn_cli_command_result(
            ...
            healthcheck,
            ["update", "99282721-934c-42be-ba4d-a93cbfda2644", "--description", "updated by me"],
        )
